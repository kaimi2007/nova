...
@require_context
def dcns_network_create(context, values):
    dcns_net_ref = models.DCNSNetwork()
    dcns_net_ref.update(values)
    dcns_net_ref.save()
    return dcns_net_ref


@require_context
def dcns_network_get(context, project, session=None):
    if not session:
        session = get_session()

    result = session.query(models.DCNSNetwork).\
                     options(joinedload('dcns_port')).\
                     filter_by(project_id=project).\
                     filter_by(deleted=False).\
                     first()

    #if not result:
    #    raise exception.DCNSNetworkNotFound(project_id=project)

    return result


@require_context
def dcns_network_update(context, project, values):
    session = get_session()
    with session.begin():
        dcns_net_ref = dcns_network_get(context, project, session=session)
        if dcns_net_ref:
            dcns_net_ref.update(values)
            dcns_net_ref.save(session=session)


@require_context
def dcns_network_delete(context, project):
    session = get_session()
    with session.begin():
        dcns_net_ref = dcns_network_get(context, project, session=session)
        if dcns_net_ref:
            if len(dcns_net_ref.dcns_port) != 0:
                for port in dcns_net_ref.dcns_port:
                    port.delete(session=session)
            dcns_net_ref.delete(session=session)


@require_context
def dcns_port_add(context, project, values):
    session = get_session()
    with session.begin():
        dcns_net_ref = dcns_network_get(context, project, session=session)
        if dcns_net_ref:
            dcns_port_ref = models.DCNSPort()
            dcns_port_ref.update(values)
            dcns_port_ref.dcns_net_id =  dcns_net_ref.id
            dcns_port_ref.save()
            return dcns_port_ref
    return None 


@require_context
def dcns_port_get(context, project, host, nic, session=None):
    if not session:
        session = get_session()

    dcns_net_ref =  dcns_network_get(context, project, session=session)
    result = session.query(models.DCNSPort).\
                     filter_by(dcns_net_id=dcns_net_ref.id).\
                     filter_by(host_name=host).\
                     filter_by(host_nic=nic).\
                     filter_by(deleted=False).\
                     first()

    #if not result:
    #    raise exception.DCNSPortNotFound(project_id=project, host_name=nost,
    #    host_nic = nic)
...
@require_context
def dcns_port_remove(context, project, host, nic):
    session = get_session()
    with session.begin():
        dcns_port_ref = dcns_port_get(context, project, host, nic, session=session)
        if dcns_port_ref:
            dcns_port_ref.delete(session=session)
