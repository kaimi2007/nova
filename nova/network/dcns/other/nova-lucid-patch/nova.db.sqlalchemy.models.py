...
class DCNSNetwork(BASE, NovaBase):
    """Represents a DCNS network."""
    __tablename__ = 'dcns_networks'
    id = Column(Integer, primary_key=True, nullable=False)
    gri = Column(String(255))
    project_id = Column(String(255), ForeignKey('projects.id'))
    bandwidth = Column(String(255))
    vlan_range = Column(String(255))
    start_time = Column(DateTime, default=utils.utcnow)
    end_time = Column(DateTime)
    status = Column(String(255))


class DCNSPort(BASE, NovaBase):
    """Represents a DCNS host-port mapping."""
    __tablename__ = 'dcns_ports'
    id = Column(Integer, primary_key=True, nullable=False)
    dcns_net_id = Column(Integer, ForeignKey('dcns_networks.id'),
nullable=True)
    dcns_network = relationship(DCNSNetwork,
                           backref=backref('dcns_port'),
                           foreign_keys=dcns_net_id,
                           primaryjoin='and_('
                                'DCNSPort.dcns_net_id == DCNSNetwork.id)')
    port_urn = Column(String(255))
    host_name = Column(String(255))
    host_nic = Column(String(255))
...
def register_models():
    ...
    models = (Service, Instance, InstanceActions, InstanceTypes,
              ...
              DCNSNetwork, DCNSPort, InstanceMetadata, Migration)
