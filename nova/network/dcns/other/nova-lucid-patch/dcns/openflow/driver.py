...
LOG = logging.getLogger("nova.network.dcns.openflow.driver")
...
class OpenFlowDriver(driver_base.L2DCNDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (url_type == 'file'):
            ...
            (ssh_host, ssh_port, dpctl_path) = url_path.split(':')
            self.dpctl = 'ssh -p ' + ssh_port + ' ' + ssh_host + ' ' + dpctl_path
    ...
    def setup_p2p_path(self, gri, hops, bw, vlan):
        ...
        LOG.debug(_("send command: %s") % sys_cmd)
        (code, result) = self.shell_execute(sys_cmd, 3)
        LOG.debug(_("result: %s") % result)
        if result.find('failed') >= 0: # TODO:  more error patterns
            return 0
