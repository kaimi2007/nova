...
flags.DEFINE_string('flat_network_dhcp_start', '140.173.210.2',
                    'Dhcp start for FlatDhcp')
...
class VlanManager(NetworkManager):
    ...
    def allocate_fixed_ip(self, context, instance_id, *args, **kwargs):
        ...
        if FLAGS.use_dcns:
            ...
            if FLAGS.vlan_interface:
                nic = FLAGS.vlan_interface
            else:
                nic = 'eth1'
            host_nics = [ instance_ref['host']+':' + nic ]
            if instance_ref['host'] != network_ref['host']:
                host_nics.append(network_ref['host']+':' + nic)
            if FLAGS.dcns_net_default_bw:
                bandwidth = FLAGS.dcns_net_default_bw
            else:
                bandwidth = '100Mbps'
            vlan = network_ref['vlan']
            ret = dcns_api.API().setup_physical_network(context, 
                project_id, host_nics, bandwidth, vlan)
            if ret == 0:
                LOG.error(_("DCNS failed to setup L2 network"))
            else:
                LOG.info(_("DCNS has set up L2 network"))
    ...
    def deallocate_fixed_ip(self, context, address, *args, **kwargs):
        ...
        if FLAGS.use_dcns:
            ret = dcns_api.API().teardown_physical_network(context, 
                                        context.project_id, forced=False)
            if ret == 0:
                LOG.error(_("DCNS failed to teardown L2 network"))
            else:
                LOG.info(_("DCNS has torn down L2 network"))
