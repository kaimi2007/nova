...
class _tilera_board(object):
    ...
    def deactivate_board(self, board_id):
        ...
        cmd = "/tftpboot/pdu_mgr 10.0.100." + str(pdu_num) + " " + str(pdu_outlet_num) + " 2 >> pdu_output"
        print cmd
        path1 = "10.0.100." + str(pdu_num)
        utils.execute('/tftpboot/pdu_mgr', path1, str(pdu_outlet_num), '2', '>>', 'pdu_output') 

        return []
        #raise

    #def activate_board(self, board_id, board_ip, name, mac_address): #MK
    def activate_board(self, board_id, board_ip, name, mac_address, ip_address): #MK
        print("activate_board \n")

        target = os.path.join(FLAGS.instances_path, name)
        print target

        cmd = "cp /tftpboot/vmlinux_" + str(board_id) + "_1 /tftpboot/vmlinux_" + str(board_id)
        print cmd
        ...
        if board_id < 5:
            ...
            pdu_num = 2
            pdu_outlet_num = board_id
        cmd = "/tftpboot/pdu_mgr 10.0.100." + str(pdu_num) + " " + str(pdu_outlet_num) + " 3 >> pdu_output"
        print cmd
        path1 = "10.0.100." + str(pdu_num) 
        utils.execute('/tftpboot/pdu_mgr', path1, str(pdu_outlet_num), '3', '>>', 'pdu_output') 
        #_MK
        ...
        cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --upload /tftpboot/tilera_fs_" + str(board_id) + ".tar.gz /tilera_fs.tar.gz --quit"
        print cmd
        path1 = "/tftpboot/tilera_fs_" + str(board_id) + ".tar.gz" 
        utils.execute('/usr/local/TileraMDE/bin/tile-monitor', '--resume', '--net', board_ip, '--upload', path1, '/tilera_fs.tar.gz', '--quit')
        #cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --upload /tftpboot/tilera_fs_" + str(board_id) + ".tar.gz /tilera_fs.tar.gz --run - mount /dev/sda1 /mnt - --wait --run - tar -xzpf /tilera_fs.tar.gz -C /mnt/ - --wait --run - cp -a /dev /mnt/ - --wait --run - rm -rf /mnt/dev/hda* /mnt/dev/sda* /mnt/dev/sdb* - --wait --quit"
        cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --run - mount /dev/sda1 /mnt - --wait --run - tar -xzpf /tilera_fs.tar.gz -C /mnt/ - --wait --quit"
        print cmd
        utils.execute('/usr/local/TileraMDE/bin/tile-monitor', '--resume', '--net', board_ip, '--run', '-', 'mount', '/dev/sda1', '/mnt', '-', '--wait', '--run', '-', 'tar', '-xzpf', '/tilera_fs.tar.gz', '-C', '/mnt/', '-', '--wait', '--quit')

        cmd = "cp /tftpboot/vmlinux_" + str(board_id) + "_2 /tftpboot/vmlinux_" + str(board_id)
        print cmd
        ...
        #cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --run - /usr/sbin/sshd - --wait -- ls | grep bin >> tile_output"
        cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --run - /usr/sbin/sshd - --wait --quit"
        print cmd
        #utils.execute('/usr/local/TileraMDE/bin/tile-monitor', '--resume', '--net', board_ip, '--run', '-', '/usr/sbin/sshd', '-', '--wait', '--', 'ls', '|', 'grep', 'bin', '>>', 'tile_output')
        utils.execute('/usr/local/TileraMDE/bin/tile-monitor', '--resume', '--net', board_ip, '--run', '-', '/usr/sbin/sshd', '-', '--wait', '--quit')
        ...
        #    cmd = "TILERA_BOARD_#" + str(board_id) + " " + board_ip + " is not ready, out_msg=" + out_msg
        #    print cmd
        #    return power_state.NOSTATE
        #else:
        cmd = "TILERA_BOARD_#" + str(board_id) + " " + board_ip + " is ready"
        print cmd
        cmd = "rm /tftpboot/vmlinux_" + str(board_id)
        print cmd
        path1 = "/tftpboot/vmlinux_" + str(board_id) 
        utils.execute('rm', path1) 
        cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --run - ifconfig xgbe0 hw ether " + mac_address + " - --wait --run - ifconfig xgbe0 " + ip_address + " - --wait --quit"
        print cmd
        utils.execute('/usr/local/TileraMDE/bin/tile-monitor', '--resume', '--net', board_ip, '--run', '-', 'ifconfig', 'xgbe0', 'hw', 'ether', mac_address, '-', '--wait', '--run', '-', 'ifconfig', 'xgbe0', ip_address, '-', '--wait', '--quit')
        cmd = "/usr/local/TileraMDE/bin/tile-monitor --resume --net " + board_ip + " --run - iptables -A INPUT -p tcp ! -s 10.0.11.1 --dport 963 -j DROP - --wait --run - rm -rf /usr/sbin/iptables* - --wait --quit" 
        print cmd
        utils.execute('/usr/local/TileraMDE/bin/tile-monitor', '--resume', '--net', board_ip, '--run', '-', 'iptables', '-A', 'INPUT', '-p', 'tcp', '!', '-s', '10.0.11.1', '--dport', '963', '-j', 'DROP', '-', '--wait', '--run', '-', 'rm', '-rf', '/usr/sbin/iptables*', '-', '--wait', '--quit') 
...
class _fake_dom(object):
    ...
    def create_domain(self, xml_dict, bpath):
        ...
        if fd <> []:
            ...
            raise
            #MK
            #raise exception.NotFound("domain with the same name already exists")
        ...
        #MK
        cmd = "mount -o loop " + bpath + "/root /tftpboot/fs_" + str(board_id)
        print cmd
        path1 = bpath + "/root"
        path2 = "/tftpboot/fs_" + str(board_id)
        utils.execute('mount', '-o', 'loop', path1, path2)
        cmd = "cd /tftpboot/fs_" + str(board_id) + "; tar -czpf ../tilera_fs_" + str(board_id) + ".tar.gz ."
        print cmd
        ...
        try:
            state = tilera_boards.activate_board(board_id, 
                         board_ip, new_dom['name'], new_dom['mac_address'], new_dom['ip_address']) #MK
                         #board_ip, new_dom['name'], new_dom['mac_address']) #MK
        except:
            self.domains.remove(new_dom) #MK
            tilera_boards.free_board(board_id)
            raise exception.NotFound("Failed to boot Tilera board %s" % board_id)
...
class tileraConnection(object):
    ...
    def __init__(self, read_only):
        ...
        self.tilera_firewall_driver = fw_class(get_connection=self._get_connection)
        ...
        #else:
        #    self.tilera_firewall_driver = utils.import_object(FLAGS.tilera_firewall_driver)
        #
        #_MK
    ...
    def spawn(self, instance):
        ...
        #                  project=project,
        #                  size=size)
        #type_data = instance_types.get_instance_type(instance['instance_type'])
    ...
    def get_info(self, instance_name):
        try:
            (state, max_mem, mem, num_cpu, cpu_time) = self._conn.get_domain_info(instance_name)
        except:
    ...
    def get_hypervisor_type(self):
        ...
        return tilera_boards.get_tilera_hw_info('hypervisor_type') #'tilera_hv'
        #_MK 
    ...
    def get_cpu_info(self):
        ...
        return tilera_boards.get_tilera_hw_info('cpu_info') #'TILEpro'
        #_MK
    ...
    def refresh_security_group_rules(self, security_group_id):
        self.tilera_firewall_driver.refresh_security_group_rules(security_group_id)

    def refresh_security_group_members(self, security_group_id):
        self.tilera_firewall_driver.refresh_security_group_members(security_group_id)
