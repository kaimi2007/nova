...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def snapshot(self, context, instance, image_href, update_task_state):
        ...
        if CONF.libvirt_type != 'lxc' and not live_snapshot:
            if state == power_state.RUNNING or state == power_state.PAUSED:
                self._detach_pci_device(virt_dom, instance['pci_devices'])
        ...
        with utils.tempdir(dir=snapshot_directory) as tmpdir:
            try:
                ...
                if CONF.libvirt_type != 'lxc' and not live_snapshot:
                    ...
                    self._attach_pci_devices(new_dom, instance['pci_devices'])
    ...
    def _soft_reboot(self, instance):
        ...
        self._prepare_pci_devices_for_use(instance['pci_devices'])
    ...
    def suspend(self, instance):
        ...
        dom = self._lookup_by_name(instance['name'])
        self._detach_pci_device(dom, instance['pci_devices'])
        dom.managedSave(0)
    ...
    def resume(self, instance, network_info, block_device_info=None):
        ...
        dom = self._create_domain_and_network(xml, instance, network_info,
                                              block_device_info)
        self._attach_pci_devices(dom, instance['pci_devices'])
    ...
    def _prepare_pci_devices_for_use(self, pci_devices):
        for dev in pci_devices:
            dbsf = pci.parse_address(dev['address'])
            libvirt_dev_addr = 'pci_%s_%s_%s_%s' % dbsf
            try:
                libvirt_dev = \
                ...
                id = dev['id']
                instance_uuid = dev['instance_uuid']
                raise exception.PciDevicePrepareFailed(id=dev['id'],
                                           instance_uuid=dev['instance_uuid'])

    def _detach_pci_device(self, dom, pci_devs):
        for dev in pci_devs:
            dom.detachDevice(self.get_guest_pci_device(dev).to_xml())
    ...
    def get_guest_pci_device(self, pci_device):
        dbsf = pci.parse_address(pci_device['address'])
        dev = vconfig.LibvirtConfigGuestPciPassthrough()
    ...
    def get_guest_config(self, instance, network_info, image_meta,
        ...
        if CONF.libvirt_type not in ('lxc', 'uml'):
