...
def pci_labels_get_all(context):
    ...
    return IMPL.pci_labels_get_all(context)


def pci_device_get(context, id):
    """Get PCI device by id."""
    return IMPL.pci_device_get(context, id)


def pci_device_get_all_by_host(context, host, status=None):
    """Get all PCI devices from all host."""
    return IMPL.pci_device_get_all_by_host(context, host, status)


def pci_device_create(context, values):
    """Create new PCI devices with values."""
    return IMPL.pci_device_create(context, values)


def pci_device_destroy(context, id):
    """Destroy PCI device by id."""
    return IMPL.pci_device_destroy(context, id)


def pci_device_destroy_all_on_host(context, host):
    """Destroy all PCI device on host."""
    return IMPL.pci_device_destroy_all_on_host(context, host)


def pci_device_attach_to_instance_by_labels(context, host, instance_uuid,
                                            labels):
    """
    Attach PCI devices with selected labels that are on selected host to
    instance with specified instance_uuid.
    """
    return IMPL.pci_device_attach_to_instance_by_labels(context, host,
                                                        instance_uuid, labels)
