...
from sqlalchemy import Integer, BigInteger, String, schema
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy import ForeignKey, DateTime, Boolean, Text, Float, Enum
...
class PciDevice(BASE, NovaBase):
    """
    Represent an PCI host devices that can be passed through to instances.
    """
    __tablename__ = 'pci_devices'
    __table_args__ = (
        Index('ix_pci_devices_host_deleted', 'host', 'deleted'),
        Index('ix_pci_devices_instance_uuid_deleted',
              'instance_uuid', 'deleted'),
        Index('ix_pci_devices_compute_id_deleted', 'compute_id', 'deleted'),
        UniqueConstraint("address", "host", "deleted",
                         name="uniq_pci_devices0host0address0deleted")
    )
    id = Column(Integer, primary_key=True)
    ...
    status = Column(Enum('in_use', 'available', 'to_delete',
    ...
    compute_id = Column(Integer, ForeignKey('compute_nodes.id'),
                        nullable=False)
    compute = relationship(ComputeNode, backref="pci_devices",
                 foreign_keys=compute_id,
                 primaryjoin='and_('
                    'PciDevice.compute_id == ComputeNode.id,'
                    'PciDevice.deleted == 0)')
