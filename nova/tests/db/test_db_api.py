...
class FixedIPTestCase(BaseInstanceTypeTestCase):
    ...
    def test_fixed_ip_get_by_address_detailed_sucsess(self):
        ...
        ignored_keys = ['info_cache', 'system_metadata',
                        'security_groups', 'metadata', 'pci_devices']
...
class ComputeNodeTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def test_compute_node_get_all(self):
        ...
        node = nodes[0]
        ignored_keys = self._ignored_keys + ['stats', 'service', 'pci_devices']
...
class PciDeviceTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def _get_ignored_keys(self):
        return ['id', 'deleted', 'deleted_at', 'updated_at', 'created_at']

    def _create_pci_device(self, values):
        vals = {
            'status': 'available',
            'label': 'fake_label',
            'host': 'fake_host',
        ...
        vals.update(values)
        return db.pci_device_create(self.ctxt, vals)

    def _create_fake_host_and_compute(self, host='host1'):
        service_dict = dict(host=host, binary='binary1',
                            topic='compute', report_count=1,
                            disabled=False)
        ...
        compute_node_dict = dict(vcpus=2, memory_mb=1024, local_gb=2048,
                                 ...
                                 running_vms=0, current_workload=0,
                                 service_id=service['id'])
        compute = db.compute_node_create(self.ctxt, compute_node_dict)
        return (service, compute)

    def _create_instances_with_args(self, **kwargs):
        args = {'reservation_id': 'a', 'image_ref': 1, 'host': 'host1',
                'node': 'node1', 'project_id': 'fake', 'vm_state': 'fake'}
        args.update(kwargs)
        return db.instance_create(self.ctxt, args)

    def test_pci_device_create(self):
        service, compute = self._create_fake_host_and_compute('host1')
        values = {
            'host': 'host1',
        ...
        pci_dev = db.pci_device_create(self.ctxt, values)
        self._assertEqualObjects(values, pci_dev,
                                 ignored_keys=self._get_ignored_keys())

    def test_pci_device_create_duplicate(self):
        service, compute = self._create_fake_host_and_compute('host1')
        values = {'address': '0000:20:00.1', 'compute_id': compute.id}
        self._create_pci_device(values)
        self.assertRaises(exception.PciDeviceAlreadyExistsOnHost,
                          self._create_pci_device, values)

    def test_pci_device_get(self):
        service, compute = self._create_fake_host_and_compute('host1')
        values = {'address': '0000:20:00.1', 'compute_id': compute.id}
        dev = self._create_pci_device(values)
        real_dev = db.pci_device_get(self.ctxt, dev['id'])
        self._assertEqualObjects(dev, real_dev,
                                 ignored_keys=self._get_ignored_keys())

    def test_pci_device_get_not_found(self):
        self.assertRaises(exception.PciDeviceNotFound,
                          db.pci_device_get, self.ctxt, 100500)

    def test_pci_device_get_all_by_host(self):
        host_addresses = {'fake1': ['1000:10:10.1', '1000:10.10.2'],
                          'fake2': ['1001:10:10.1', '1001.10.10.2]']}
        host_devices = {}

        for host, addresses in host_addresses.iteritems():
            service, compute = self._create_fake_host_and_compute(host=host)
            host_devices[host] = []
            for address in addresses:
                values = {'address': address,
                          'host': host,
                          'compute_id': compute['id']}
                host_devices[host].append(self._create_pci_device(values))

        for host, devices in host_devices.iteritems():
            real_devices = db.pci_device_get_all_by_host(self.ctxt, host=host)
            self._assertEqualListsOfObjects(real_devices, devices)

    def test_pci_device_get_all_by_host_and_status(self):
        addresses = ['1000:10:10.1', '1000:10.10.2', '1000:10.10.3']
        statuses = ['in_use', 'to_delete', 'available']

        host_devices = {}
        for host in ['host1', 'host2']:
            service, compute = self._create_fake_host_and_compute(host=host)
            host_devices[host] = []
            for address, status in zip(addresses, statuses):
                values = {'address': address,
                          'host': host,
                          'compute_id': compute['id'],
                          'status': status}
        ...
        for host, devices in host_devices.iteritems():
            for status in statuses:
                expected = filter(lambda d: d['status'] == status, devices)
                real = db.pci_device_get_all_by_host(self.ctxt, host, status)
                self._assertEqualListsOfObjects(expected, real,
                                                ignored_keys=ignored_keys)
    ...
    def test_pci_device_destroy(self):
        ...
        values = [{'address': '0000:20:00.1', 'compute_id': compute.id,
                   'status': 'available'},
                  {'address': '0000:20:00.2', 'compute_id': compute.id,
                   'status': 'in_use'}]

        devs = [self._create_pci_device(vals) for vals in values]
        for dev in devs:
            db.pci_device_destroy(self.ctxt, dev['id'])

        self.assertRaises(exception.PciDeviceNotFound,
                          db.pci_device_get, self.ctxt, devs[0]['id'])
        dev = db.pci_device_get(self.ctxt, devs[1]['id'])
        self.assertEqual(dev['status'], 'to_delete')

    def test_pci_device_destroy_not_found(self):
        self.assertRaises(exception.PciDeviceNotFound,
                          db.pci_device_destroy, self.ctxt, 100500)

    def test_pci_device_destroy_all_on_host(self):
        hosts = ['host1', 'host2']

        host_devs = {}
        for host in hosts:
            service, compute = self._create_fake_host_and_compute(host)
            values = [{'address': '0000:20:00.1', 'compute_id': compute.id,
                       'status': 'available', 'host': host},
                      {'address': '0000:20:00.2', 'compute_id': compute.id,
                       'status': 'in_use', 'host': host}]
            host_devs[host] = [self._create_pci_device(vals)
                                    for vals in values]

        db.pci_device_destroy_all_on_host(self.ctxt, hosts[0])
        devices = db.pci_device_get_all_by_host(self.ctxt, hosts[0])
        self.assertEqual(len(devices), 1)
        self.assertEqual(devices[0]['status'], 'to_delete')
        ignored_keys = self._get_ignored_keys() + ['status']
        self._assertEqualObjects(devices[0], host_devs[hosts[0]][1],
                                 ignored_keys=ignored_keys)

        devices = db.pci_device_get_all_by_host(self.ctxt, hosts[1])
        self._assertEqualListsOfObjects(host_devs[hosts[1]], devices,
                                        ignored_keys=self._get_ignored_keys())

    def _prepare_pci_device_attach_to_instance_by_labels(self):
        hosts = ['fake_host1', 'fake_host2']
        services = []
        computes = []
        for host in hosts:
            service_compute = self._create_fake_host_and_compute(host=host)
            services.append(service_compute[0])
            computes.append(service_compute[1])

        instance = self._create_instances_with_args(display_name='test1')

        pci_dev_refs = []
        addresses = ['1000:10:10:1', '1000:10:10:2', '1000:10:10:3']
        for i, host in enumerate(hosts):
            compute_id = computes[i]['id']
            for k, address in enumerate(addresses):
                ...
                pci_dev_refs.append(self._create_pci_device(pci_values))

        return hosts, services, computes, instance, pci_dev_refs

    def test_pci_device_attach_to_instance_by_labels(self):
        data = self._prepare_pci_device_attach_to_instance_by_labels()
        hosts, services, computes, instance, pci_devices = data

        attach_labels = ['a0', 'a1']

        db.pci_device_attach_to_instance_by_labels(self.ctxt, hosts[0],
                                                   instance['uuid'],
                                                   attach_labels)

        devs_in_use = db.pci_device_get_all_by_host(self.ctxt, hosts[0],
                                                    status='in_use')

        real_attached_labels = map(lambda x: x['label'], devs_in_use)
        self._assertEqualListsOfPrimitivesAsSets(real_attached_labels,
                                                 attach_labels)

        for host in hosts[1:]:
            devs_in_use = db.pci_device_get_all_by_host(self.ctxt, host,
                                                        status='in_use')
            self.assertEqual(len(devs_in_use), 0)

    def test_pci_device_attach_to_instance_with_nonexist_labels(self):
        attach_labels = ['a0', 'non_existing_uuid']

        data = self._prepare_pci_device_attach_to_instance_by_labels()
        hosts, services, computes, instance, pci_devices = data

        self.assertRaises(exception.PciDeviceBulkAttachFailed,
                          db.pci_device_attach_to_instance_by_labels,
                          self.ctxt, hosts[0], instance['uuid'], attach_labels)
    ...
    def test_pci_devices_attach_to_instance_with_already_used_labels(self):
        attach_labels = ['a0', 'a1']
        ...
        instance2 = self._create_instances_with_args(display_name='test2')

        self.assertRaises(exception.PciDeviceBulkAttachFailed,
                          db.pci_device_attach_to_instance_by_labels,
                          self.ctxt, hosts[0], instance2['uuid'],
                          attach_labels)
        ...
        devs_in_use = db.pci_device_get_all_by_host(self.ctxt, hosts[0],
                                                    status='in_use')
        self.assertEqual(len(attach_labels), len(devs_in_use))
        for dev in devs_in_use:
            self.assertEqual(dev['instance_uuid'], instance['uuid'])

    def test_pci_devices_attach_to_instance_by_labels_nonexist_instance(self):
        attach_labels = ['a0', 'a1']
        ...
        self.assertRaises(exception.InstanceNotFound,
                          db.pci_device_attach_to_instance_by_labels,
                          self.ctxt, hosts[0], 'non_existing_uuid',
                          attach_labels)

    def test_attached_pci_devices_are_released_after_instance_destroy(self):
        service, compute = self._create_fake_host_and_compute()
        instance = self._create_instances_with_args(display_name='test1')
        pci_dev_values = [
            {'address': '1000:10:10:1',
             'compute_id': compute['id'],
             'instance_uuid': instance['uuid'],
             'status': 'in_use'},
            {'address': '1000:10:10:2',
             'compute_id': compute['id'],
             'instance_uuid': instance['uuid'],
             'status': 'to_delete'}
        ]
        pci_devs = [self._create_pci_device(vals) for vals in pci_dev_values]
        db.instance_destroy(self.ctxt, instance['uuid'])

        updated_dev = db.pci_device_get(self.ctxt, pci_devs[0]['id'])
        self.assertEqual(updated_dev['instance_uuid'], None)
        self.assertEqual(updated_dev['status'], 'available')
        self.assertRaises(exception.PciDeviceNotFound,
                          db.pci_device_get, self.ctxt, pci_devs[1]['id'])

    def test_pci_device_destroyed_by_compute_node_delete(self):
        service1, compute1 = self._create_fake_host_and_compute(host='h1')
        service2, compute2 = self._create_fake_host_and_compute(host='h2')

        compute1_devs = []
        compute2_devs = []
        for address in ['0000:00:00.1', '0000:00:00.2', '0000:00:00.3']:

            compute1_devs.append(
                self._create_pci_device({'compute_id': compute1['id'],
                                         'host': 'h1',
                                         'address': address}))
            compute2_devs.append(
                self._create_pci_device({'compute_id': compute2['id'],
                                         'host': 'h2',
                                         'address': address}))

        db.compute_node_delete(self.ctxt, compute1['id'])
        for dev in compute1_devs:
            self.assertRaises(exception.PciDeviceNotFound,
                              db.pci_device_get, self.ctxt, dev['id'])

        real_compute2_devs = db.pci_device_get_all_by_host(self.ctxt, 'h2')
        self._assertEqualListsOfObjects(compute2_devs, real_compute2_devs,
                                        ignored_keys=self._get_ignored_keys())

    def test_instance_has_pci_devices(self):
        service, compute = self._create_fake_host_and_compute()
        instance = self._create_instances_with_args(display_name='test1')
        pci_dev_values = {
            'address': '1000:10:10:1',
        ...
        pci_dev_ref = self._create_pci_device(pci_dev_values)
        instance = db.instance_get(self.ctxt, instance['id'])
        self._assertEqualListsOfObjects(instance['pci_devices'], [pci_dev_ref],
                                        self._get_ignored_keys())
