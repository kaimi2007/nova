...
binary_name = linux_net.binary_name
...
class IptablesManagerTestCase(test.TestCase):
    ...
    def test_filter_rules_are_wrapped(self):
        ...
        self.assertTrue('-A %s-FORWARD '
                        '-s 1.2.3.4/5 -j DROP' % binary_name in new_lines)

        table.remove_rule('FORWARD', '-s 1.2.3.4/5 -j DROP')
        new_lines = self.manager._modify_rules(current_lines, table)
        self.assertTrue('-A %s-FORWARD '
                        '-s 1.2.3.4/5 -j DROP' % binary_name not in new_lines)
    ...
    def test_nat_rules(self):
        ...
        for chain in ['POSTROUTING', 'PREROUTING', 'OUTPUT']:
            self.assertTrue('-A %s -j %s-%s' \
                            % (chain, binary_name, chain) in new_lines,
    ...
    def test_filter_rules(self):
        ...
        self.assertTrue('-A nova-filter-top '
                        '-j %s-local' % binary_name in new_lines,
