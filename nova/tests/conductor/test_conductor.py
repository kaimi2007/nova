...
class _BaseTestCase(object):
    ...
    def test_pci_device_create(self):
        ...
        self.mox.StubOutWithMock(db, 'pci_device_create')
        db.pci_device_create(ctxt, 'fake-values').AndReturn('fake-result')
        self.mox.ReplayAll()
        result = self.conductor.pci_device_create(ctxt, 'fake-values')
        self.assertEqual(result, 'fake-result')

    def test_pci_device_destroy_all_on_host(self):
        ctxt = context.get_admin_context()
        self.mox.StubOutWithMock(db, 'pci_device_destroy_all_on_host')
        db.pci_device_destroy_all_on_host(ctxt, 'fake-host').AndReturn(None)
        self.mox.ReplayAll()
        result = self.conductor.pci_device_destroy_all_on_host(ctxt,
                                                               'fake-host')
        self.assertEqual(result, None)

    def test_pci_device_attach_to_instance_by_labels(self):
        ctxt = context.get_admin_context()
        self.mox.StubOutWithMock(db, 'pci_device_attach_to_instance_by_labels')
        host = 'fake'
        instance_uuid = 'some_instance_uuid'
        labels = ['a', 'b']
        db.pci_device_attach_to_instance_by_labels(ctxt, host, instance_uuid,
                                                   labels).AndReturn(None)
        self.mox.ReplayAll()
        result = self.conductor.pci_device_attach_to_instance_by_labels(ctxt,
                                                host, instance_uuid, labels)
        self.assertEqual(result, None)
