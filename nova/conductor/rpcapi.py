...
class ConductorAPI(nova.openstack.common.rpc.proxy.RpcProxy):
    ...
    1.53 - Added pci_device_create, pci_device_destroy_all_on_host,
                 instance_attach_pci_devices_with_labels
    """
    ...
    def pci_device_create(self, context, values):
        msg = self.make_msg('pci_device_create', values=values)
        return self.call(context, msg, version='1.53')

    def pci_device_destroy_all_on_host(self, context, host):
        msg = self.make_msg('pci_device_destroy_all_on_host', host=host)
        return self.call(context, msg, version='1.53')

    def pci_device_attach_to_instance_by_labels(self, context, host,
                                                instance_uuid, labels):
        msg = self.make_msg('pci_device_attach_to_instance_by_labels',
                             host=host, instance_uuid=instance_uuid,
                             labels=labels)
        return self.call(context, msg, version='1.53')
