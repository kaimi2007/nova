...
class ConductorManager(manager.Manager):
    ...
    @rpc_common.client_exceptions(exception.PciDeviceAlreadyExistsOnHost,
                                  exception.PciDeviceWrongAddressFormat)
    def pci_device_create(self, context, values):
        return self.db.pci_device_create(context, values)

    def pci_device_destroy_all_on_host(self, context, host):
        self.db.pci_device_destroy_all_on_host(context, host)

    @rpc_common.client_exceptions(exception.PciDeviceBulkAttachFailed,
                                  exception.InstanceNotFound)
    def pci_device_attach_to_instance_by_labels(self, context, host,
                                                instance_uuid, labels):
        self.db.pci_device_attach_to_instance_by_labels(context, host,
                                                        instance_uuid,
                                                        labels)
